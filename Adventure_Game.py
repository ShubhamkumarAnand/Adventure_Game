...
def cave(item, option):
    if "sward" in item:
        ...
        print_pause("\nYour eye catches a glint of metal behind a "
                    "rock.")
        print_pause("\nYou have found the magical Sword of India!")
...
def house(item, option):
    ...
    print_pause("\nOh No! This is the " + option + "'s house!")
    ...
    while True:
        ...
        if choice2 == "1":
            if "sward" in item:
                ...
                print_pause("\nThe Sword of India shines brightly in "
...
def play_game():
    item = []
    option = random.choice(["Thanos", "pirate of Bengal", "Metal Man", "Deadly Terrorist",
                            "Angry Wild Animal"])
    intro(item, option)
    field(item, option)
